    ...
    async for message in websocket:
        ...
        if message[11] == "l":
            ...
            if new_minute[0] == True and get_current_second() >= 30:
                ...
                if get_current_second() < 30:
                    trade_seconds.append(get_current_second() + 30)
                else:
                    trade_seconds.append(get_current_second() - 30)
                trade_minutes.clear()
                ...
                print(
                    ...
                    tracker_dict["Last Transaction Price"][-1],
                )
                ...
                if (
                    ...
                    and current_price
                    >= tracker_dict["Last Transaction Price"][-1] * 1.05
                ):
                    cancel_stop_loss()
                    sell()
                ...
                if (
                    ...
                    and current_price
                    <= tracker_dict["Last Transaction Price"][-1] * 0.97
                ):
                    cancel_stop_loss()
                    buy()
                    transaction_price = get_transaction_price()
...
def main():
    ...
    pseudo_log_return = math.log(average_price) - math.log(
        tracker_dict["Average Price"][-1]
    )
    ...
    tracker_dict["Trend Indicator"].append(
        estimate_coef(trade_seconds[:-1], minute_trade_prices[:-1])[1]
    )
    print(tracker_dict)
    ...
    features = [
        [
            tracker_dict["Hour"][-3],
            tracker_dict["Minute"][-3],
            tracker_dict["Average Price"][-3],
            tracker_dict["Standard Deviation"][-3],
            tracker_dict["Pseudo-Log-Return"][-3],
        ],
        [
            tracker_dict["Hour"][-2],
            tracker_dict["Minute"][-2],
            tracker_dict["Average Price"][-2],
            tracker_dict["Standard Deviation"][-2],
            tracker_dict["Pseudo-Log-Return"][-2],
        ],
        [
            tracker_dict["Hour"][-1],
            tracker_dict["Minute"][-1],
            tracker_dict["Average Price"][-1],
            tracker_dict["Standard Deviation"][-1],
            tracker_dict["Pseudo-Log-Return"][-1],
        ],
    ]
    features_df = pd.DataFrame(
        {
            ...
            "Trend Indicator": [
                tracker_dict["Trend Indicator"][-3],
                tracker_dict["Trend Indicator"][-2],
                tracker_dict["Trend Indicator"][-1],
            ],
        }
    )
    ...
    if len(predictions) > 0:
        if (
            minute_trade_prices[-1] <= minute_trade_prices[0]
            and predictions[-1] == True
        ):
            correct_predictions.append(1)
            print("Accuracy: ", sum(correct_predictions) / len(correct_predictions))
        elif (
            minute_trade_prices[-1] >= minute_trade_prices[0]
            and predictions[-1] == False
        ):
            correct_predictions.append(1)
            print("Accuracy: ", sum(correct_predictions) / len(correct_predictions))
        else:
...
def is_stop_limit():
    if get_orders()[0]["order_type"] == "stop_limit":
        return True
    else:
        return False
...
def replace_stop_loss(stop_price, limit_price):
    ...
    order_endpoint = "https://paper-api.alpaca.markets/v2/orders/{}".format(
        stop_loss_id
    )
    data = {"qty": current_qty[0], "limit_price": limit_price, "stop_price": stop_price}
...
def get_standard_deviation(average_price):
    ...
    for price in minute_trade_prices:
        sum_transaction_price_minus_average_price_squared += (
            price - average_price
        ) ** 2
    return math.sqrt(
        sum_transaction_price_minus_average_price_squared / len(minute_trade_prices)
    )
...
if __name__ == "__main__":
    ...
    tracker_dict = {
        ...
        "Pseudo-Log-Return": [0, 0, 0],
        "Trend Indicator": [0, 0, 0],
        "Last Transaction Price": [0, 0, 0],
    }
