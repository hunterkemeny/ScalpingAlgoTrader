...
def get_model():
    ...
    tuner = kt.Hyperband(model_builder, executions_per_trial=1, 
                        objective='val_accuracy', max_epochs=500, factor=2, hyperband_iterations=3, 
                        distribution_strategy=strategy, tune_new_entries=True,
                        allow_new_entries=True,
                        directory='DNN_Hypersearch', project_name='multi-layer')
    tuner.search_space_summary()
    # Use the early stopping callback after 20 iterations.
    tuner.search(trainX, trainY, epochs = 300, batch_size=BATCH_SIZE, validation_data = (validX, validY), 
...
def model_builder(hp):
    ...
    for i in range(hp.Int('num_layers', 2, 10)):
        ...
        x = layers.Dense(units=hp.Int('units_' + str(i),
                        ...
                        kernel_regularizer=regularizers.l2(hp.Float('kernel_l2', max_value=1e-2, min_value=1e-5, sampling='log')),
                        bias_regularizer=regularizers.l2(hp.Float('bias_l2', max_value=1e-2, min_value=1e-5, sampling='log')),
                        activity_regularizer=regularizers.l2(hp.Float('activity_l2', max_value=1e-2, min_value=1e-5, sampling='log')))(x)
        ...
        x = layers.Dropout(hp.Float('dropout_' + str(i), max_value=0.7, min_value=0.1, sampling='linear'))(x)
    ...
    lr_schedule = keras.optimizers.schedules.InverseTimeDecay(
                    initial_learning_rate=hp.Float('initial_learning_rate', max_value=1, min_value=1e-4, sampling='log'),
                    decay_steps=23000/512 * 1000,
                    decay_rate=hp.Float('decay_rate', max_value=1, min_value=0.1, sampling='linear'))
...
if __name__ == "__main__":
    ...
    lstm_train_pred_df = pd.DataFrame(lstm_train_pred.reshape(trainY.shape[0], trainY.shape[1]))
    # Output all predictions to a CSV file.
    lstm_train_pred_df.to_csv('./CleanData/dnn_train_pred.csv', index=False)
    lstm_train_pred = pd.read_csv('./CleanData/dnn_train_pred.csv', error_bad_lines=False)
    ...
    print('AUC Score of DNN on valid: ', roc_auc_score(np.array(y_true), lstm_valid_pred.reshape(lstm_valid_pred.shape[0], lstm_valid_pred.shape[1])))
    lstm_valid_pred_df = pd.DataFrame(lstm_valid_pred.reshape(validY.shape[0], validY.shape[1]))
    # Output all predictions to a CSV file.
    lstm_valid_pred_df.to_csv('./CleanData/dnn_valid_pred.csv', index=False)
    lstm_valid_pred = pd.read_csv('./CleanData/dnn_valid_pred.csv', error_bad_lines=False)
    ...
    print('AUC Score of DNN on test: ', roc_auc_score(np.array(y_true), lstm_test_pred.reshape(lstm_test_pred.shape[0], lstm_test_pred.shape[1])))
    lstm_test_pred_df = pd.DataFrame(lstm_test_pred.reshape(testY.shape[0], testY.shape[1]))
    # Output all predictions to a CSV file.
